(self["webpackChunk_hydrooj_ui_default"] = self["webpackChunk_hydrooj_ui_default"] || []).push([["default-components_messagepad_reducers_index_ts-components_problemconfig_reducer_index_ts-com-d6dead"],{
...
function reducer(state = {}, action = {}) {
  ...
  switch (action.type) {
    case "DIALOGUES_LOAD_DIALOGUES_FULFILLED": {
      ...
      return __spreadValues(__spreadValues({}, state), lodash__WEBPACK_IMPORTED_MODULE_0___default().keyBy(messages, "udoc._id"));
    }
    ...
    case "DIALOGUES_POST_SEND_FULFILLED": {
      ...
      return __spreadProps(__spreadValues({}, state), {
        [id]: __spreadProps(__spreadValues({}, state[id]), {
          messages: [
            ...state[id].messages,
            mdoc
          ]
    ...
    case "DIALOGUES_MESSAGE_PUSH": {
      ...
      return __spreadProps(__spreadValues({}, state), {
        [to]: __spreadProps(__spreadValues({}, state[to] || {}), {
          udoc,
          messages: [
...
/* harmony import */ var _activeId__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./activeId */ "./components/messagepad/reducers/activeId.ts");
/* harmony import */ var _dialogues__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ./dialogues */ "./components/messagepad/reducers/dialogues.js");
/* harmony import */ var _inputs__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! ./inputs */ "./components/messagepad/reducers/inputs.js");
/* harmony import */ var _isPosting__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! ./isPosting */ "./components/messagepad/reducers/isPosting.ts");
...
const reducer = (0,redux__WEBPACK_IMPORTED_MODULE_4__.combineReducers)({
  activeId: _activeId__WEBPACK_IMPORTED_MODULE_0__["default"],
  dialogues: _dialogues__WEBPACK_IMPORTED_MODULE_1__["default"],
  inputs: _inputs__WEBPACK_IMPORTED_MODULE_2__["default"],
  isPosting: _isPosting__WEBPACK_IMPORTED_MODULE_3__["default"]
});
...
function reducer(state = {}, action = {}) {
  switch (action.type) {
    case "DIALOGUES_LOAD_DIALOGUES_FULFILLED": {
      ...
      return __spreadValues(__spreadValues({}, state), lodash__WEBPACK_IMPORTED_MODULE_0___default().fromPairs(lodash__WEBPACK_IMPORTED_MODULE_0___default().map(dialogues, (d) => [d._id, ""])));
    }
    case "DIALOGUES_CREATE": {
      const { user } = action.payload;
      return __spreadProps(__spreadValues({}, state), {
...
function reducer(state = {}, action = {}) {
  switch (action.type) {
    case "DIALOGUES_LOAD_DIALOGUES_FULFILLED": {
      const dialogues = action.payload.messages;
      return __spreadValues(__spreadValues({}, state), lodash__WEBPACK_IMPORTED_MODULE_0___default().fromPairs(lodash__WEBPACK_IMPORTED_MODULE_0___default().map(dialogues, (d) => [d._id, false])));
    }
    case "DIALOGUES_CREATE": {
      const { user } = action.payload;
      return __spreadProps(__spreadValues({}, state), {
...
function reducer(state = {
  ...
  switch (action.type) {
    case "CONFIG_LOAD_FULFILLED": {
      const c = __spreadProps(__spreadValues({}, state), { __loaded: true });
      try {
    ...
    case "CONFIG_FORM_UPDATE": {
      const next = __spreadProps(__spreadValues({}, state), { [action.key]: action.value });
      if (action.key === "score" && action.value)
        next.score = +next.score;
      if (action.key === "checker_type" && action.value === "other")
        next.checker_type = "syzoj";
      if (!action.value || typeof action.value === "object" && !action.value.join(""))
    ...
    case "CONFIG_CODE_UPDATE": {
      try {
        ...
        return __spreadProps(__spreadValues(__spreadValues({}, state), data), {
          __valid: true,
          __errors: [],
          __loaded: true
        });
    ...
    case "CONFIG_AUTOCASES_UPDATE": {
      ...
      for (const subtask of subtasks) {
        if (subtask.time === (0,_hydrooj_utils_lib_common__WEBPACK_IMPORTED_MODULE_0__.parseTimeMS)(state.time || "1s"))
          delete subtask.time;
        if (subtask.memory === (0,_hydrooj_utils_lib_common__WEBPACK_IMPORTED_MODULE_0__.parseMemoryMB)(state.memory || "256m"))
          delete subtask.memory;
        if (subtask.time)
          subtask.time += "ms";
        if (subtask.memory)
          subtask.memory += "MB";
      ...
      if (subtasks.length === 0)
        ...
        next.subtasks = subtasks.map((subtask) => __spreadValues(__spreadValues({}, subtask), { cases: subtask.cases.map((i) => ({ input: i.input, output: i.output })) }));
      }
      return next;
    }
    case "problemconfig/updateFileIO": {
      const n = __spreadProps(__spreadValues({}, state), { filename: action.filename });
      if (!n.filename)
        delete n.filename;
      return n;
    }
    case "problemconfig/updateGlobalConfig": {
      const n = __spreadProps(__spreadValues({}, state), { time: action.time, memory: action.memory });
      if (!n.time)
        delete n.time;
      if (!n.memory)
        delete n.memory;
    ...
    case "problemconfig/addSubtask": {
      ...
      return __spreadProps(__spreadValues({}, state), { subtasks });
    }
    case "problemconfig/updateSubtaskConfig": {
      if (!state.subtasks.find((i) => i.id === action.id))
        return state;
      const subtask = __spreadValues({}, state.subtasks.find((i) => i.id === action.id));
      const subtasks = [...state.subtasks];
      subtasks.splice(state.subtasks.findIndex((i) => i.id === action.id), 1, subtask);
      if (action.payload.time)
        subtask.time = action.payload.time;
      if (action.payload.memory)
        subtask.memory = action.payload.memory;
      if (action.payload.score)
        subtask.score = +action.payload.score || 0;
      if (action.payload.if)
        subtask.if = action.payload.if;
      if (action.payload.type)
        subtask.type = action.payload.type;
      if (!subtask.time)
        delete subtask.time;
      if (!subtask.memory)
        delete subtask.memory;
    ...
    case "problemconfig/addTestcases": {
      return __spreadProps(__spreadValues({}, state), { __cases: (0,_hydrooj_utils_lib_common__WEBPACK_IMPORTED_MODULE_0__.sortFiles)([...state.__cases, ...action.cases], "input") });
    }
    case "problemconfig/moveTestcases": {
      ...
      return __spreadProps(__spreadValues({}, state), { subtasks, __cases });
    }
    case "problemconfig/delTestcases": {
      const testcases = action.cases;
      return __spreadProps(__spreadValues({}, state), { __cases: state.__cases.filter((i) => !testcases.find((j) => i.input === j.input && i.output === j.output)) });
    }
...
/* harmony import */ var _config__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./config */ "./components/problemconfig/reducer/config.ts");
/* harmony import */ var _testdata__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ./testdata */ "./components/problemconfig/reducer/testdata.ts");



const reducer = (0,redux__WEBPACK_IMPORTED_MODULE_2__.combineReducers)({
  config: _config__WEBPACK_IMPORTED_MODULE_0__["default"],
  testdata: _testdata__WEBPACK_IMPORTED_MODULE_1__["default"]
});
/* harmony default export */ const __WEBPACK_DEFAULT_EXPORT__ = (reducer);
...
if (((_a = UiContext.tdoc) == null ? void 0 : _a._id) && UiContext.tdoc.rule !== "homework")
  cacheKey += `@${UiContext.tdoc._id}`;
function reducer(state = {
...
/* harmony import */ var _editor__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./editor */ "./components/scratchpad/reducers/editor.ts");
/* harmony import */ var _pretest__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ./pretest */ "./components/scratchpad/reducers/pretest.ts");
/* harmony import */ var _records__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! ./records */ "./components/scratchpad/reducers/records.ts");
/* harmony import */ var _state__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! ./state */ "./components/scratchpad/reducers/state.ts");
/* harmony import */ var _ui__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(/*! ./ui */ "./components/scratchpad/reducers/ui.ts");
...
const reducer = (0,redux__WEBPACK_IMPORTED_MODULE_5__.combineReducers)({
  ...
  pretest: _pretest__WEBPACK_IMPORTED_MODULE_1__["default"],
  records: _records__WEBPACK_IMPORTED_MODULE_2__["default"],
...
/* harmony export */   "default": () => (/* binding */ reducer)
/* harmony export */ });
/* harmony import */ var _hydrooj_utils_lib_status__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! @hydrooj/utils/lib/status */ "../utils/lib/status.ts");
var __defProp = Object.defineProperty;
var __defProps = Object.defineProperties;
var __getOwnPropDescs = Object.getOwnPropertyDescriptors;
var __getOwnPropSymbols = Object.getOwnPropertySymbols;
...
function reducer(state = {
  ...
  if (action.type === "SCRATCHPAD_RECORDS_PUSH") {
    ...
    if (rdoc._id === state.rid) {
      const output = [`${_hydrooj_utils_lib_status__WEBPACK_IMPORTED_MODULE_0__.STATUS_TEXTS[rdoc.status]} ${rdoc.time}ms ${rdoc.memory}KiB`];
      if (rdoc.compilerTexts.length)
        output.push(rdoc.compilerTexts.join("\n"));
      if (rdoc.testCases.length)
        output.push(rdoc.testCases[0].message || "");
      return __spreadProps(__spreadValues({}, state), {
        output: output.join("\n")
      });
    }
...
function reducer(state = {
  ...
  switch (action.type) {
    case "SCRATCHPAD_UI_SET_VISIBILITY": {
      ...
      return __spreadProps(__spreadValues({}, state), {
        [uiElement]: __spreadProps(__spreadValues({}, state[uiElement]), {
          visible: visibility
        })
      });
    }
    ...
    case "SCRATCHPAD_SETTING_UPDATE": {
      ...
      const config = __spreadProps(__spreadValues({}, state.settings.config), {
        [setting]: value
      });
      localStorage.setItem("editor.config", JSON.stringify(config));
      return __spreadProps(__spreadValues({}, state), {
        settings: __spreadProps(__spreadValues({}, state.settings), {
          config
        })
      });
    }
    ...
    case "SCRATCHPAD_POST_SUBMIT_FULFILLED": {
      ...
      return action.type === "SCRATCHPAD_POST_SUBMIT_FULFILLED" && UiContext.canViewRecord ? __spreadProps(__spreadValues({}, state), {
        records: __spreadProps(__spreadValues({}, state.records), {
          visible: true
        }),
        isPosting: false,
        submitWaitSec: 8
    ...
    case "SCRATCHPAD_WAITING_TICK": {
      if (Date.now() - state.lastTick < 950)
        return state;
      return __spreadProps(__spreadValues({}, state), {
        lastTick: Date.now(),
